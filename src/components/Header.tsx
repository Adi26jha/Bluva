  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ${
        isScrolled ? 'bg-navy-base/80 backdrop-blur-lg py-4 border-b border-white/10' : 'bg-transparent py-6'
      }`}
    >
          ...
          {navLinks.map((link) => (
            <Link
              ...
              className={`relative text-sm font-medium tracking-wide uppercase transition-colors duration-300 ${
                location.pathname === link.path ? 'text-brand-blue' : 'text-white/70 hover:text-white'
              }`}
            >
              {link.name}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="absolute top-full left-0 w-full bg-navy-base border-b border-white/10 py-8 px-6 md:hidden flex flex-col gap-6 items-center"
          >
            {navLinks.map((link) => (
              <Link
                key={link.name}
