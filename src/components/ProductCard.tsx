...
const ProductCard = ({ id, name, size, image, description }: ProductCardProps) => {
  return (
    <motion.div
      whileHover={{ y: -10 }}
      className="glass-card p-6 w-[320px] shrink-0 border-white/5 group relative overflow-hidden h-[450px] flex flex-col"
    >
      ...
      <div className="mt-auto space-y-2">
        <div className="flex justify-between items-start">
          <h3 className="text-2xl font-editorial font-bold text-white">{name}</h3>
        ...
        <Link 
          to={`/products#${id}`}
          className="inline-block pt-4 text-xs font-bold uppercase tracking-[0.2em] text-accent-cyan hover:text-white transition-colors"
        >
          Learn More +
        </Link>
      </div>
